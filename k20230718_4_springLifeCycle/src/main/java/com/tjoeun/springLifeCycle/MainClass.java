...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		// 1. refresh() 메소드는 load() 메소드로 DI 컨테이너에 넣어준 bean 설정 정보에 따른 bean이 일괄적으로 생성한다.
		// 2. refresh() 메소드를 실행하지 않으면 getBean() 메소드로 DI 컨테이너에 해당 bean을 얻어오는 순간 bean이 생성된다.
		...
		Person person2 = ctx.getBean("person2", Person.class);
		System.out.println(person2);
		// refresh() 메소드를 실행했더라도 close() 메소드를 실행하지 않으면 destory() 메소드는 실행되지 않는다.
		// close() 메소드를 실행했더라도 refresh() 메소드를 실행하지 않았다면 destory() 메소드는 실행되지 않는다. 
		ctx.close();
